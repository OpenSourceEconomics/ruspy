...
def get_table(init_dict):
    """
    This function produces a latex table with replication results for group 4 with
    different statesizes.

    :param init_dict: A dictionary containing a section with the group name,
    the binsize, discount factor beta,
...
def get_repl_result(init_dict):
    ...
    if not os.path.isfile(
        data_path + "/pkl/group_data/" + group + "_" + binsize + ".pkl"
    ):
        data_reading()

    if not os.path.isfile(
        data_path + "/pkl/replication_data/rep_" + group + "_" + binsize + ".pkl"
    ):
