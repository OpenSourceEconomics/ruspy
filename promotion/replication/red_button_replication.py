...
if not os.path.isfile(data_path + '/pkl/group_data/' + group + '.pkl'):
    data_reading()

if not os.path.isfile(data_path + '/pkl/replication_data/rep_' + group + '.pkl'):
    data = data_processing(init_dict['replication'])
else:
    data = pd.read_pickle(data_path + '/pkl/replication_data/rep_' + group + '.pkl')
