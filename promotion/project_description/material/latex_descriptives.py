...
def create_desc():
    ...
    for filename in list_pkl:
        dict_df[filename[0:7]] = pd.read_pickle(data_path + '/pkl/group_data/' +
                                                filename)
    ...
    for j, i in enumerate(sorted(dict_df.keys())):
        ...
        df3 = dict_df[i][['Odo_2nd']].sub(dict_df[i]['Odo_1st'],
                                          axis=0)[dict_df[i]['Odo_2nd'] > 0]
        ...
        if j == 0:
            ...
            df_ges = pd.concat([df_ges,
                                df4.rename(columns={group_name: 'Full sample'})],axis=0)
    df = pd.concat([df, df_ges.describe()], axis=1)
    df = df.transpose()
    df = df.drop(df.columns[[4, 5, 6]], axis=1)
    df = df[['max', 'min', 'mean', 'std', 'count']].fillna(0).astype(int)
    df.rename(columns={'max': 'Max', 'min': 'Min', 'mean': 'Mean',
                       'std': 'Std. Dev.', 'count': 'NumObs'}, inplace=True)
    ...
    for i in sorted(dict_df.keys()):
        ...
        if j == 0:
            ...
            df_ges = pd.concat([df_ges, df2.rename(columns={group_name: 'Full sample'})], axis=0)
    df = pd.concat([df, df_ges.describe()], axis=1)
    df = df.transpose()
    df = df.drop(df.columns[[4, 5, 6]], axis=1)
    df = df[['max', 'min', 'mean', 'std', 'count']].fillna(0).astype(int)
